...
start = mdo
    ...
    emit_ (Mov (Ptr Bottom) (Deref $ Imm 0) (Direct $ Imm 0))
...
alloc = mdo
    ...
    nextAvailableAddr <- emit (\r -> Mov (Ptr Bottom) (dreg r) (Deref $ Imm 0))
    -- increment, and store
    emit_ (Add (Ptr Bottom) nextAvailableAddr (dreg objsize))
    emit_ (Mov (Ptr Bottom) (Deref $ Imm 0) (dreg nextAvailableAddr))
    -- copy object to the place
    -- TODO: loop if object is big
    emit_ (Mov Bottom (Deref $ Register nextAvailableAddr) (Deref $ Register obj))
